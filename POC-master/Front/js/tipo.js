...
const getIngresso = async (id) => {
    const response = await axios.get(`${ENDPOINT}tipos/${id}`)
    return response.data
}

const ingressoDelete = async (id) => {
    ...
    axios.delete(`${ENDPOINT}ingressos/` + id)
        .then((response) => {
            Swal.fire(`Ingresso '${ingresso.id}' deleted`)
            loadTable();
        }, (error) => {
            Swal.fire(`Erro ao deletar o tipo ${error.response.data.error} `);
            loadTable();
            return;
        });

    Swal.fire(`Tipo '${ingresso.id}' deletado`)
    loadTable();
};
...
const loadTable = (sort, type) => {
    if (type)
    {
        axios.get(`${ENDPOINT}tipos?limit=1000&type=${type}&sort=${sort}`)
        .then((response) => {
            ...
            data.forEach(element => {
                ...
                trHTML += '<td><button type="button" class="btn btn-outline-secondary" onclick="showIngressoEditBox(' + element.id + ')">Edit</button>';
                trHTML += '<button type="button" class="btn btn-outline-danger" onclick="ingressoDelete(' + element.id + ')">Del</button></td>';
                trHTML += "</tr>";
            });
    ...
    {
        axios.get(`${ENDPOINT}tipos?limit=1000&sort=${sort}`)
        .then((response) => {
            const data = response.data;
            var trHTML = '';
            data.forEach(element => {
    ...
    Swal.fire(
        {
            ...
            html:
                `<form id="swal-form">` +
                    ...
                    `<div>` +
                        ...
                        `<input name="percentual_ingresso" placeholder="percentual_ingresso" />` +
                    `</div>` +
                `</form>`,
            focusConfirm: false,
            ...
            preConfirm: async () =>
            {
                try
                {
                    ...
                    const data = await createNew('tipos', form)
...
const showIngressoEditBox = async (id) => {
    const tipos = await getTipos()
    const categorias = await getCategorias()
    const ingresso = await getIngresso(id)
    ...
    Swal.fire(
        {
            ...
            html:
                `<form id="swal-form">` +
                    `<input name="id" type="hidden" value="${ingresso.id}" />` +
...
const createNew = async (path, form) =>
{
    ...
    const result = await axios.post(`${ENDPOINT}tipos`, newObj)
...
const edit = async (path, form) =>
{
    ...
    const result = await axios.put(`${ENDPOINT}tipos/${id}`, newObj)
