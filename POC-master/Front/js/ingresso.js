...
const getIngressos = async () => {
    const response = await axios.get(`${ENDPOINT}ingressos`)
...
const getIngresso = async (id) => {
    const response = await axios.get(`${ENDPOINT}ingressos/${id}`)
    return response.data
}
...
const ingressoDelete = async (id) => {
    ...
    axios.delete(`${ENDPOINT}ingressos/` + id)
        .then((response) => {
            Swal.fire(`Ingresso '${ingresso.id}' deletado`)
            loadTable();
        }, (error) => {
            Swal.fire(`Erro ao deletar o ingresso ${error.response.data.error} `);
            loadTable();
            return;
        });
...
const loadTable = (sort, type) => {
    if (type)
    {
        axios.get(`${ENDPOINT}ingressos?limit=1000&type=${type}&sort=${sort}`)
        .then((response) => {
            ...
            data.forEach(element => {
                ...
                trHTML += '<td>' + element.data + '</td>';
                trHTML += '<td>' + element.valor + '</td>';
                trHTML += '<td>' + element.Tipo.descricao + '</td>';
                trHTML += '<td>' + element.Categoria.descricao + '</td>';
    ...
    {
        axios.get(`${ENDPOINT}ingressos?limit=1000&sort=${sort}`)
        .then((response) => {
            const data = response.data;
            var trHTML = '';
            data.forEach(element => {
    ...
    Swal.fire(
        {
            ...
            preConfirm: async () =>
            {
                try
                {
                    ...
                    const data = await createNew('ingressos', form)
